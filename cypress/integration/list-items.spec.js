describe('List items', () => {
    ...
    it('properly displays completed items', () => {
        cy.get('.todo-list li').filter('.completed').should('have.length', 1).and('contain', 'Eggs').find('.toggle').should('be.checked')
    })
    ...
    it('Removes a todo', () => {
        ...
        cy.get('@list').first().find('.destroy').invoke('show').click({force: true});
